param (
    $resourceBaseName="storybook$( Get-Random -Maximum 1000)",
    $location='eastus'
)
...
dotnet build ..\src\Storybook.Api\Storybook.Api.csproj

Write-Host 'Packaging API project for deployment' -ForegroundColor Green
dotnet publish ..\src\Storybook.Api\Storybook.Api.csproj --self-contained -r win-x86 -o ..\publish
